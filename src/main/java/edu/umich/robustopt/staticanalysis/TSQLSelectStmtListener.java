...
class TSQLSelectStmtListener extends Antlr4TSQLAnalyzerBaseListener {
    ...
    private class ColumnResolver {
        ...
        public void resolveName(ColumnNameInfo c) {
            ...
            //if (c.getQueryColumnName().equals("c_count"));
    ...
    public class QueryInfo {
        private List<ColumnNameInfo> select = new ArrayList<>();
        ...
        public List<ColumnNameInfo> getSelectColumnInfo() { return select; }
        ...
        public List<Pair<String, String>> getSelectColumnStrings() { return select.stream().map(Columns::getTrueName).collect(Collectors.toList()); }
        ...
        public List<Pair<String, String>> getUnionColumnStrings() {
            List<Pair<String, String>> res = new ArrayList<>();
            res.addAll(getSelectColumnStrings());
            res.addAll(getFromColumnStrings());
            res.addAll(getWhereColumnStrings());
            res.addAll(getGroupByColumnStrings());
            res.addAll(getOrderByColumnStrings());
            return res;
        }
    ...
    @Override
    public void enterColumn_alias_list(Column_alias_listContext ctx) {
        for (int i=0; i<ctx.column_alias().size(); i++)
            lastDerivedSymbols.get(i).setColumnAlias(ctx.column_alias(i).getText());
    }
    ...
    public void exitQuery_specification(Query_specificationContext ctx) {
        ...
        List<ColumnNameInfo> selectInfo = swgoTop.getSelectColumnInfo();
        List<ColumnNameInfo> expandedSelectInfo = new ArrayList<>();
        List<ColumnNameInfo> expandedInfo = new ArrayList<>();
        ...
        for (ColumnNameInfo c : selectInfo)
            ...
            else expandedSelectInfo.add(c);

        //selectInfo.removeIf(x->x.getQueryColumnName().equals("*"));
        //selectInfo = expandedSelectInfo; //.addAll(expandedInfo);
        selectInfo.clear();
        selectInfo.addAll(expandedSelectInfo);
        ...
        symbolStack.pop();
        if (nestedLevel>0 && SQLContextStack.peek().getType() == SQLContext.ClauseType.FROM)
            for (ColumnNameInfo c : selectInfo) {
